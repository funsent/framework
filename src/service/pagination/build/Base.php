...
class Base
{
    ...
    {
        $self           = max(Request::get('paginate', 1), 1);
    ...
    private function setUrl()
    {
        ...
        return $this->url = "?{$url}paginate={paginate}";
    }

    //获取URL地址
    private function getUrl($num)
    {
        return str_replace('{paginate}', $num, $this->url);
    }

    //获取URL前部分
    private function getUrlBefore()
    {
        return substr($this->url, 0, strpos($this->url, '{paginate}'));
    }

    //获取URL后部分
    private function getUrlEnd()
    {
        return substr($this->url, -strpos($this->url, '{paginate}'));
    }
    ...
    public function select()
    {
        ...
        $str
            = "<select name='paginate' class='page_select' onchange='
        javascript:
    ...
    {
        $str
            = "<input id='pagekeydown' type='text' name='paginate' value='{$this->selfPage}' class='pageinput' onkeydown = \"javascript:
        if(event.keyCode==13){
    ...
    public function pres()
    {
        ...
        return $this->selfPage > $this->pageNum ? "<li><a href='".$this->getUrl(
                $num
            )."' class='pres'>前{$this->pageNum}页</a></li>" : "";
    }
